...
public class MapView extends JPanel {
  ...
  private final String[] startingTutorialImages = {
  ...
  private final String[] talentTreeTutorial = {
      "TalentPointTutorial.png",
      "TalentTreeTutorial.png",
      "ImmuneLevel1MobTutorial.png"
  };

  private final String[] batMobTutorial = {
  ...
  private final TDButton backButton;
  ...
  private void changeTutorial() {
    ...
    this.backButton.setVisible(this.imageWalkthrough != 0);

    this.continueButton.setVisible(this.imageWalkthrough != this.activeTutorial.length - 1);
  }
...
//			}
    ...
    if (model.allTowers() != null) {
      for (final Tower tower : model.allTowers()) {
        ...
        if (tower.getShowRange()) {
          ...
          g2.fillOval(tower.getX() + (tower.getWidth() / 2) - range,
              tower.getY() + (tower.getHeight() / 2) - range, range * 2, range * 2);
        }
      }
    }

    if (this.getMousePosition() != null &&
        this.view.getSidePanelView().getTowerView().getBuildButtonGroup().getSelectedButton() !=
            null) {

      int range = 0;
      switch (this.view.getSidePanelView().getTowerView().getBuildButtonGroup().getSelectedButton()
          .getName()) {
        case "DamageTower":
  ...
  public boolean canBuild(Position p) {
    ...
    boolean bottomRight =
        isGreen(getRGB(x + TowerPanelView.TOWER_ICON_WIDTH, y + TowerPanelView.TOWER_ICON_HEIGHT));
