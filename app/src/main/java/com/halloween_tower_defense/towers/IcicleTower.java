...
public class IcicleTower extends Tower {
  ...
  public IcicleTower(final Position location, final GameModel model) {
  ...
  public Projectile shootMob(final GameModel model) {
    Vector vector =
        new Vector(this.position, this.attackingMob[0].getPosition(), Icicle.PROJECTILE_SPEED);
    ...
    return new Icicle(model, this.position, trajectory, this.path3UpgradeLevel,
