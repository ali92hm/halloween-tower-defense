...
public class FreezeTower extends Tower {
  ...
  public Projectile[] attackMob(GameModel model) {
    ...
    this.reloadProgress =
        (int) (((this.fireRate) - (100 * this.path2UpgradeLevel)) * fireRateBoost);
    ...
    for (Mob mob : model.allMobs()) {
      if (this.position.getDistance(mob.getPosition()) < (((this.range + (25 *
        ...
        if (mob.getDistanceTraveled() > this.mobTravelDistance) {
          projectiles[0] = new DeepFreeze(model, this.position, null, this.path3UpgradeLevel,
