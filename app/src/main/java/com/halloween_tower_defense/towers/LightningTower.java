...
public class LightningTower extends Tower {
  ...
  public Projectile[] attackMob(final GameModel model) {
    ...
    if (chainLightning && this.attackingMob[0] != null) {
      for (Mob mob : model.allMobs()) {
        if (this.position.getDistance(mob.getPosition()) <
            (this.range * rangeBoost) + mob.getRadius() &&
  ...
  public Projectile shootMob(final GameModel model) {
    ...
    return new ThunderBolt(model, this.position, trajectory, this.path3UpgradeLevel,
        this.path1UpgradeLevel, false);
  }
  ...
  public int path1CurrentValue() {
    return (getChainLightningUpgrade() ?
        ChainLightning.getDamageLevelBoost(this.path1UpgradeLevel) :
  ...
  public int path1UpgradeValue() {
    return this.path1UpgradeLevel == 3 ? -1 :
        (getChainLightningUpgrade() ?
            ChainLightning.getDamageLevelBoost(this.path1UpgradeLevel + 1) :
